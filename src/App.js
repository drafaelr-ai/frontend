...
// ===================================
// COMPONENTE: CRONOGRAMA DE COMPRAS
// ===================================
const CronogramaCompras = ({ obraSelecionada, voltarParaLista }) => {
    ...
    const [alertas, setAlertas] = useState({ proximas: [], atrasadas: [] });
    ...
    const carregarCompras = async () => {
        try {
            setLoading(true);
            const response = await fetchWithAuth(`${API_URL}/obras/${obraSelecionada.id}/compras`);
    ...
    const carregarAlertas = async () => {
        try {
            const response = await fetchWithAuth(`${API_URL}/obras/${obraSelecionada.id}/compras/alertas`);
            const data = await response.json();
            setAlertas(data);
        } catch (error) {
    ...
    const abrirModalEditar = (compra) => {
        ...
        setFormData({
            ...
            fornecedor_sugerido: compra.fornecedor_sugerido || '',
            valor_estimado: compra.valor_estimado,
            data_prevista: compra.data_prevista,
            categoria: compra.categoria || 'Material',
            prioridade: compra.prioridade || 3,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = compraEditando
                ? `${API_URL}/obras/${obraSelecionada.id}/compras/${compraEditando.id}`
                : `${API_URL}/obras/${obraSelecionada.id}/compras`;
            ...
            if (response.ok) {
                ...
                alert('Erro ao salvar compra: ' + (error.erro || 'Erro desconhecido'));
            }
        } catch (error) {
    ...
    const handleDeletar = async (compraId) => {
        if (!window.confirm('Deseja realmente deletar esta compra agendada?')) return;

        try {
            const response = await fetchWithAuth(
                `${API_URL}/obras/${obraSelecionada.id}/compras/${compraId}`,
                { method: 'DELETE' }
            );
    ...
    const handleMarcarRealizada = async (compraId) => {
        const valorReal = prompt('Digite o valor real pago (deixe em branco para usar o valor estimado):');
        
        try {
            const body = valorReal ? { valor_real: parseFloat(valorReal) } : {};
            const response = await fetchWithAuth(
                `${API_URL}/obras/${obraSelecionada.id}/compras/${compraId}/marcar-realizada`,
                {
                    method: 'POST',
            ...
            if (response.ok) {
                ...
                alert('Erro ao marcar compra como realizada');
            }
        } catch (error) {
    ...
    const comprasFiltradas = compras.filter(compra => {
        const statusMatch = filtroStatus === 'Todos' || compra.status === filtroStatus;
        const categoriaMatch = filtroCategoria === 'Todas' || compra.categoria === filtroCategoria;
        return statusMatch && categoriaMatch;
    });

    const comprasAgrupadas = comprasFiltradas.reduce((grupos, compra) => {
        const mes = compra.data_prevista.substring(0, 7);
        if (!grupos[mes]) grupos[mes] = [];
        grupos[mes].push(compra);
        return grupos;
    }, {});
    ...
    const getStatusColor = (status) => {
        switch(status) {
            ...
            case 'Realizada': return 'var(--cor-acento)';
            case 'Atrasada': return 'var(--cor-vermelho)';
    ...
    return (
        <div className="dashboard-container">
            <div className="dashboard-header">
                <div>
                    <h1>📅 Cronograma de Compras</h1>
                    <p>{obraSelecionada.nome}</p>
                </div>
                <button onClick={voltarParaLista} className="voltar-btn">← Voltar</button>
            </div>
                ...
                <div style={{ marginBottom: '20px' }}>
                    {alertas.atrasadas.length > 0 && (
                        <div style={{
                            ...
                            <h3 style={{ margin: '0 0 10px 0', color: 'var(--cor-vermelho)' }}>
                                🚨 {alertas.atrasadas.length} Compra(s) Atrasada(s)
                            </h3>
                            {alertas.atrasadas.map(compra => (
                                <div key={compra.id} style={{ fontSize: '0.9em', marginBottom: '5px' }}>
                                    • <strong>{compra.item}</strong> - Previsto: {new Date(compra.data_prevista + 'T00:00:00').toLocaleDateString('pt-BR')}
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    {alertas.proximas.length > 0 && (
                        <div style={{
                            ...
                            <h3 style={{ margin: '0 0 10px 0', color: '#f59e0b' }}>
                                ⚠️ {alertas.proximas.length} Compra(s) Próxima(s) (7 dias)
                            </h3>
                            {alertas.proximas.map(compra => (
                                <div key={compra.id} style={{ fontSize: '0.9em', marginBottom: '5px' }}>
                                    • <strong>{compra.item}</strong> - {new Date(compra.data_prevista + 'T00:00:00').toLocaleDateString('pt-BR')}
                                </div>
                            ))}
                        </div>
                    )}
            ...
            <div className="card-full">
                <div className="card-header">
                    ...
                    <div className="header-actions">
                        ...
                        {(user.role === 'administrador' || user.role === 'master') && (
                            <button onClick={abrirModalNovo} className="acao-btn add-btn">
                                + Nova Compra
                            </button>
                        )}
                    </div>
                ...
                <div style={{ padding: '20px' }}>
                    {Object.keys(comprasAgrupadas).length === 0 ? (
                        ...
                        Object.keys(comprasAgrupadas).sort().map(mesAno => (
                            <div key={mesAno} style={{ marginBottom: '30px' }}>
                                ...
                                <div style={{ display: 'grid', gap: '15px' }}>
                                    {comprasAgrupadas[mesAno].map(compra => (
                                        <div
                                            key={compra.id}
                                            style={{
                                                border: '1px solid #e0e0e0',
                                                borderLeft: `4px solid ${getStatusColor(compra.status)}`,
                                                borderRadius: '8px',
                                                padding: '15px',
                                                backgroundColor: '#fff',
                                                transition: 'box-shadow 0.2s'
                                            }}
                                            onMouseEnter={(e) => e.currentTarget.style.boxShadow = 'var(--sombra-card)'}
                                            onMouseLeave={(e) => e.currentTarget.style.boxShadow = 'none'}
                                        >
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '10px' }}>
                                                <div style={{ flex: 1 }}>
                                                    <h4 style={{ margin: '0 0 5px 0', color: 'var(--cor-texto)' }}>
                                                        {compra.item}
                                                    </h4>
                                                    <div style={{ fontSize: '0.85em', color: 'var(--cor-texto-secundario)' }}>
                                                        📅 {new Date(compra.data_prevista + 'T00:00:00').toLocaleDateString('pt-BR')}
                                                        {compra.categoria && ` • ${compra.categoria}`}
                                                        {compra.fornecedor_sugerido && ` • ${compra.fornecedor_sugerido}`}
                                                    </div>
                                                </div>

                                                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                                    <PrioridadeBadge prioridade={compra.prioridade} />
                                                    <span
                                                        style={{
                                                            backgroundColor: getStatusColor(compra.status),
                                                            color: 'white',
                                                            padding: '4px 10px',
                                                            borderRadius: '12px',
                                                            fontSize: '0.8em',
                                            ...
                                            {compra.descricao && (
                                                <p style={{ margin: '10px 0', fontSize: '0.9em', color: 'var(--cor-texto-secundario)' }}>
                                                    {compra.descricao}
                                                </p>
                                            )}
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
                                                <div>
                                                    <strong style={{ fontSize: '1.1em', color: 'var(--cor-primaria)' }}>
                                                        {formatCurrency(compra.valor_estimado)}
                                                    </strong>
                                                    {compra.valor_real && (
                                                        <span style={{ marginLeft: '10px', fontSize: '0.85em', color: 'var(--cor-acento)' }}>
                                                            Real: {formatCurrency(compra.valor_real)}
                                                        </span>
                                                    )}
                                                </div>
                                                ...
                                                {(user.role === 'administrador' || user.role === 'master') && (
                                                    <div style={{ display: 'flex', gap: '8px' }}>
                                                        {compra.status === 'Pendente' && (
                                                            <button
                                                                onClick={() => handleMarcarRealizada(compra.id)}
                                                                style={{
                                                        ...
                                                        <button
                                                            onClick={() => handleDeletar(compra.id)}
            ...
            {modalAberto && (
                <div className="modal-overlay" onClick={() => setModalAberto(false)}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => setModalAberto(false)} className="close-modal-btn">×</button>
                        <h2>{compraEditando ? 'Editar Compra' : 'Nova Compra Agendada'}</h2>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="form-group">
                                ...
                                <input
                                    ...
                                    required
                                    placeholder="Ex: Cimento CP-II, Areia fina, etc."
                                />
                            </div>
                            ...
                            <div className="form-actions">
                                <button type="button" onClick={() => setModalAberto(false)} className="cancel-btn">
                                    Cancelar
                                </button>
                                <button type="submit" className="submit-btn">
                                    {compraEditando ? 'Salvar Alterações' : 'Criar Compra'}
                                </button>
                            </div>
                        </form>
                    </div>
...
function Dashboard() {
    ...
    // VERIFICAÇÃO DO CRONOGRAMA DE COMPRAS
    if (telaCronograma) {
