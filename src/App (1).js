...
const CronogramaFinanceiro = ({ onClose, obraId, obraNome }) => {
    ...
    const handleDeletePagamentoFuturo = async (id) => {
        // Verificar se é um pagamento de serviço (IDs começam com "servico-")
        if (String(id).startsWith('servico-')) {
            alert('Pagamentos de serviços não podem ser excluídos aqui. Exclua o pagamento no card do serviço.');
            return;
        }
                        ...
                        <table className="tabela-pendencias">
                            ...
                            <tbody>
                                {pagamentosFuturos.map(pag => {
                                    const isServico = String(pag.id).startsWith('servico-');
                                    const idParaSelecao = isServico ? pag.pagamento_servico_id : pag.id;
                                    const tipoParaSelecao = isServico ? 'servico' : 'futuro';
                                        ...
                                        <tr key={pag.id}>
                                            <td>
                                                {pag.status === 'Previsto' && (
                                                    <input 
                                                        type="checkbox" 
                                                        checked={isItemSelecionado(tipoParaSelecao, idParaSelecao)}
                                                        onChange={() => toggleSelecao(tipoParaSelecao, idParaSelecao)}
                                                        style={{ cursor: 'pointer', width: '18px', height: '18px' }}
                                                    />
                                                )}
                                            </td>
